...
docker --config target/docker/"${TARGET_REGISTRY}"/"${TARGET_IMAGE}"/GPT-POC-"${IMAGE_TAG}"/docker \
...
docker --config target/docker/127.0.0.1/5000/"${TARGET_IMAGE}"/GPT-POC-"${IMAGE_TAG}"/docker \
buildx create --use --name "${BUILDER_NAME}" --driver-opt network=host \
--buildkitd-flags '--allow-insecure-entitlement security.insecure --allow-insecure-entitlement network.host'