 ...
 * <p>
 * These tests use an unusual structure where the bulk of the code is stored in lambdas within the {@#testWhenAndThenSteps} method.
 * All the "when" steps are run in the `@BeforeClass` method and then the "then" steps are run in the individual `@Test` methods.
 * This structure saves about 1 second per test over having the "when" steps in the `@Test` methods.
 */
...
public class PathUpdateTests extends AbstractTestNGSpringContextTests
{
    ...
    /** All the "when" and "then" steps for each test, grouped according to the test name. */
    private Map<String, Pair<Supplier<Map<String, Object>>, Consumer<Map<String, Object>>>> testWhenAndThenSteps = Map.of(
            "testChangeFileNameUpdatesPath", Pair.of(() -> {
            ...
            "testChangeFileParentUpdatesPath", Pair.of(() -> {
            ...
            "testChangeFolderNameUpdatesPath", Pair.of(() -> {
            ...
            "testChangeFolderParentUpdatesPath", Pair.of(() -> {
            ...
            "testChangeCategoriesUpdatesPath", Pair.of(() -> {
            ...
            "testChangeCategoryNameUpdatesPath", Pair.of(() -> {
            ...
            "testChangeCategoryPathUpdatesPath", Pair.of(() -> {
                ...
                return Map.of("fileName", testFile.getName(), "childCategoryName", childCategory.getName(),"parentCategoryName", parentCategory.getName());
            }, (properties) -> {
                String fileName = (String) properties.get("fileName");
    ...
    public void dataPreparation()
    {
        ...
        assertEquals("Some entries in testWhenAndThenSteps do not have corresponding @Test methods.", emptySet(), Sets.difference(testWhenAndThenSteps.keySet(), testMethods));

        // Execute the "when" step for all tests and store the state to be passed into the "then" steps.
        propertyMaps = testWhenAndThenSteps.entrySet()
                                           .stream()
    ...
    public void testChangeFileNameUpdatesPath()
    {
        ...
        testWhenAndThenSteps.get(methodName).getRight().accept(propertyMaps.get(methodName));
    }
